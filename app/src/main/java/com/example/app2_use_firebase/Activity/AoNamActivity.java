...
public class AoNamActivity extends BaseActivity{
    ActivityListAoNamBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        binding = ActivityListAoNamBinding.inflate(getLayoutInflater());
            ...
            public void run() {
                try {
                    ...
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            if (itemsAoNamList != null && !itemsAoNamList.isEmpty()) {
                                items.addAll(itemsAoNamList);

                                if (!items.isEmpty()) {
                                    binding.recyclerViewListAoNam.setLayoutManager(new GridLayoutManager(AoNamActivity.this, 2));
                                    binding.recyclerViewListAoNam.setAdapter(new PopularAdapter(items));
                                }

                                binding.progressAo.setVisibility(View.GONE);
                            } else {
                                Toast.makeText(AoNamActivity.this, "Web service connect error", Toast.LENGTH_LONG).show();
                            }
                        }
                    });
                } catch (Exception e) {
                        ...
                        public void run() {
                            if (itemsSliderItems != null && !itemsSliderItems.isEmpty()) {
                                ...
                                if (!imageUrls.isEmpty()) {
                                    SliderImgAdapter slideAdapter = new SliderImgAdapter(AoNamActivity.this, imageUrls);
